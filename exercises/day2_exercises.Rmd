...
foreach (comm_csv = comm_csv_list, .combine = 'c') %dopar% {
  comm <- read.csv(comm_csv, row.names = 1)
  nmds <- vegan::metaMDS(comm, trace = FALSE)
  Sys.sleep(5) # added to extend the running time of the small example
...
filepath <- "/scratch/project_2020485/<your folder here>/stop_times_day.csv"

tic()
times_read.csv <- read.csv(filepath)
toc()

tic()
times_read_csv <- read_csv(filepath)
toc()

tic()
times_arrow_tibble <- read_csv_arrow(filepath)
toc()

tic()
times_arrow_table <- read_csv_arrow(filepath, as_data_frame = FALSE)
toc()
```
...
```{r}
# Let's remove the existing copy of the data
rm(hsl_day)
...
# Load the partitioned Parquet data and the file stops.txt from the original HSl data
hsl_day <- open_dataset("/scratch/project_2020485/<your folder here>/stop_times.parquet")
hsl_stops <- open_csv_dataset("/scratch/project_2020485/shared_data/hsl/stops.txt")
